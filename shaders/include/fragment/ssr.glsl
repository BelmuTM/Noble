...
vec3 getSkyFallback(vec2 coords, vec3 reflected) {
    return (texture(colortex7, projectSphere(normalize(mat3(gbufferModelViewInverse) * reflected)) * ATMOSPHERE_RESOLUTION).rgb + sun(reflected, shadowDir)) * getSkyLightmap(coords);
}
