...
void main() {
    ...
    vec4 GlobalIllumination = vec4(0.0f);
    #if SSGI == 1
    ...
    vec3 Diffuse = Albedo * (GlobalIllumination.rgb + (NdotL * Shadow));
    vec3 Specular = vec3(0.0f);
    vec3 specColor = specularFresnelSchlick(mix(Albedo, skyColor, NdotL), NdotL) * Shadow;
